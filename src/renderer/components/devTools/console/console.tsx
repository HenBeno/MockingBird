...
export function Console({ search }: Props) {
  ...
  const [parentData, setParentData] = useState<Partial<RouteParent> | null>(
    null,
  );
  const [isRouteDialogOpen, setIsRouteDialogOpen] = useState(false);
  const [isQueryDialogOpen, setIsQueryDialogOpen] = useState(false);
  const [isQueryResponseDialogOpen, setIsQueryResponseDialogOpen] =
    useState(false);
  const [routeData, setRouteData] = useState<Partial<Route> | null>(null);
  const [queryData, setQueryData] = useState<Partial<GraphQlRoute> | null>(
    null,
  );
  const [_serverName, setServerName] = useState<string | null>(null);
  const [isResponseDialogOpen, setIsResponseDialogOpen] = useState(false);
  const [responseData, setResponseData] =
    useState<Partial<RouteResponse> | null>(null);
  const [queryResponseData, setQueryResponseData] =
    useState<Partial<GraphQlRouteResponse> | null>(null);

  const [openRows, setOpenRows] = useState<{ [key: string]: boolean }>({});
  const server = serversHash[_serverName || ''];
  ...
  const handleAddQueryResponseClick = ({
    data,
    matchedParent,
    matchedRoute,
    serverName,
  ...
  return (
    <>
      ...
      {isResponseDialogOpen && !!route && !!parent && !!server && (
        <ResponseDialog
          onClose={closeDialogs}
          open
          data={responseData}
      ...
      {isQueryDialogOpen && !!parent && !!server && (
        <GraphqlRouteDialog
          open
          onClose={closeDialogs}
          data={queryData}
      ...
      {isQueryResponseDialogOpen && !!parent && !!query && !!server && (
